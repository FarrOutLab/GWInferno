...
def convert_component_spins_to_chieff(dat_array, param_names, injections = False):
    ...
    if injections:
        chi_eff_array = xr.DataArray(chi_eff.reshape(1,chi_eff.shape[0]), dims = ['param', 'injection'], coords = {'param': ['chi_eff'], 'injection': np.arange(dat_array.injection.shape[0])})
        prior_array = xr.DataArray(new_prior.reshape(1,chi_eff.shape[0]), dims = ['param', 'injection'], coords = {'param': ['prior'], 'injection': np.arange(dat_array.injection.shape[0])})
        new_arrays.append(chi_eff_array)
        new_arrays.append(prior_array)

        if chip:
            chip_array = xr.DataArray(chi_p.reshape(1,chi_p.shape[0]), dims = ['param', 'injection'], coords = {'param': ['chi_p'], 'injection': np.arange(dat_array.injection.shape[0])})
            new_arrays.append(chip_array)

    else:
        chi_eff_array = xr.DataArray(chi_eff.reshape(chi_eff.shape[0], 1,chi_eff.shape[1]), dims = ['event', 'param', 'samples',], coords = {'event': dat_array.event,'param': ['chi_eff'], 'samples': dat_array.samples})
        prior_array = xr.DataArray(new_prior.reshape(prior.shape[0],1,prior.shape[1]), dims = ['event', 'param', 'samples'], coords = {'event': dat_array.event, 'param': ['prior'], 'samples': dat_array.samples})
        new_arrays.append(chi_eff_array)
        new_arrays.append(prior_array)

        if chip:
            chip_array = xr.DataArray(chi_p.reshape(1,chi_p.shape[0]), dims = ['event', 'param', 'samples'], coords = {'event': dat_array.event,'param': ['chi_p'], 'samples': dat_array.samples})
            new_arrays.append(chip_array)
