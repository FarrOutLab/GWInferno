...
def setup_bspline_spin_models_2d(pedict, injdict, a1_nsplines, ct1_nsplines, IID=False, a2_nsplines=None, ct2_nsplines=None):
    print("initializing spline spin design tensors")

    if IID:
        spin_mag_tilt_model = BivariateBSplineIIDSpinMagTilt(ndofs=(a1_nsplines, ct1_nsplines),
                                    primary_pe_vals=(pedict["a_1"], pedict["cos_tilt_1"]), primary_inj_vals=(injdict["a_1"], injdict["cos_tilt_1"]),
                                    secondary_pe_vals=(pedict["a_2"], pedict["cos_tilt_2"]), secondary_inj_vals=(injdict["a_2"], injdict["cos_tilt_2"]),
                                    normalize=True)
    return spin_mag_tilt_model
...
def bspline_mass_prior(m_nsplines=None, q_nsplines=None, m_tau=1, q_tau=1, name=None, m_cs_sig=15, q_cs_sig=5, m_deg=2, q_deg=2):
...
def bspline_spin_prior_2d(a_nsplines=None, ct_nsplines=None, a_tau=None, ct_tau=None, name=None, IID=False, a_cs_sig=5, ct_cs_sig=5, a_deg=2, ct_deg=2):
    ...
    if IID:
        ...
        a_ct_cs = numpyro.sample("a_ct_cs" + name, dist.Dirichlet(jnp.ones((a_nsplines,ct_nsplines), dtype='float32')))
        numpyro.factor("a_ct_smoothing_prior" + name, apply_2d_difference_prior(a_ct_cs, a_tau, order=a_deg))
        return a_ct_cs
