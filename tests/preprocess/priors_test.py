...
def test_chi_effective_prior_from_aligned_spins():
    ...
    assert_allclose(p_chi_eff, 0.0, err_msg="chi_eff prior not 0. at the bounds for q=1")
    p_chi_eff = priors.chi_effective_prior_from_aligned_spins(chi_eff_bounds, q=0.5)
    assert_allclose(p_chi_eff, 0.0, err_msg="chi_eff prior not 0. at the bounds for q=0.5")
    ...
    chi_eff = np.array([-1.0, -0.5, 0.0, 0.5, 1.0])
