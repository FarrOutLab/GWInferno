...
class TestBase1DBSplineModel(unittest.TestCase):
    ...
    def test_PLPeakPrimaryBSplineRatio(self):
        ...
        pe_pdf = plpeak_spline(pe_m1, alpha=-3, mmin=self.mmin, mmax=self.mmax, peak_mean=10.0, peak_sd=3.0, peak_frac=0.8, coefs=self.coefs, pe_samples=True)
        inj_pdf = plpeak_spline(inj_m1, alpha=-3, mmin=self.mmin, mmax=self.mmax, peak_mean=10.0, peak_sd=3.0, peak_frac=0.8, coefs=self.coefs, pe_samples=False)
