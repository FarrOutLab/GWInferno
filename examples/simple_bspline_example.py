...
from gwinferno.pipeline.utils import load_base_parser, bspline_mass_prior, bspline_spin_prior, bspline_redshift_prior, posterior_dict_to_xarray, pdf_dict_to_xarray, load_pe_and_injections_as_dict
from gwinferno.postprocess.calculations import calculate_bspline_mass_ppds, calculate_bspline_spin_ppds, calculate_powerlaw_spline_rate_of_z_ppds
...
from utils import setup_result_dir, run_analysis
...
def model(pedict, injdict, Nobs, Tobs, Ninj, mass_models, mag_model, tilt_model, z_model, mmin, mmax, nspline_dict, param_names):
    ...
    a1_cs, tilt1_cs, a2_cs, tilt2_cs = bspline_spin_prior(a_nsplines=nspline_dict['a1'], ct_nsplines=nspline_dict['tilt1'], a_tau = 25, ct_tau=25, IID = False)
...
def main():
    ...
    for i in range(len(names)):
        mass, m1s, mass_ratio, qs = calculate_bspline_mass_ppds(posterior[f'mass_cs'].values, posterior[f'q_cs'].values, nspline_dict, args.mmin, args.mmax)
    ...
    tilt2_pdfs = []
    for i in range(len(names)):
        mag1, mag2, mags, tilt1, tilt2, tilts = calculate_bspline_spin_ppds(posterior[f'a1_cs'].values, posterior[f'tilt1_cs'].values, nspline_dict, a2_cs = posterior[f'a2_cs'].values, tilt2_cs = posterior[f'tilt2_cs'].values)
    ...
    """
    pdf_dict = {'a1': mag1_pdfs[0], 'cos_tilt1': tilt1_pdfs[0], 'a2': mag2_pdfs[0], 'cos_tilt2': tilt2_pdfs[0], 'mass_1': mass_pdfs[0], 'mass_ratio': q_pdfs[0], 'redshift': r_of_z}
