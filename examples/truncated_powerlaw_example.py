...
def calculate_m1q(alpha, beta, mmin, mmax, rate=None):
    ...
    def calc_pdf(a, b, mi, ma, r):
        ...
        p_mq = jnp.where(jnp.less(mm, mi) | jnp.less(mm * qq, mi), 0, p_mq)
    ...
    for ii in trange(alpha.shape[0]):
        mpdfs[ii], qpdfs[ii] = calc_pdf(alpha[ii], beta[ii], mmin, mmax, rate[ii])
...
def main():
    args = load_parser()
    label = f"{args.outdir}/truncated_powerlaw_m1q_powerlaw_z"
